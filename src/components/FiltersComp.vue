<template>
	<div class="filters-component">
		<div :class="isLabelVisible && 'flex items-center'">
			<div v-if="isLabelVisible" class="text-14 ob-text-accent-1 q-mr-md">Filters:</div>
			<div class="flex justify-end">
				<q-select
					v-if="isSelectVisible"
					class="minmax-width ob-border-radius-50 ob-border-accent-0 ob-select-field height-31 q-mr-md"
					dense
					borderless
					v-model="type"
					...
					options-selected-class="ob-selected-class"
					behavior="menu"
				/>
				<q-input
					...
					} ob-border-radius-50 ob-border-accent-0 ob-input-field height-${fieldHeight} use-input q-px-md ${
					...
					><template v-slot:append>
						<q-icon
							...
							name="close"
							@click="fetchAll()"
							class="cursor-pointer ob-text-accent-0"
						/>
						<q-icon
							@click="searchKeyword()"
							class="ob-text-accent-0 cursor-pointer"
				...
				<q-select
					...
					option-value="value"
					option-label="label"
					:class="`minmax-width ob-border-accent-0 ob-border-radius-50 ob-select-field height-31 text-14 ${
						isLabelVisible && 'q-mr-md'
					}`"
				...
				>
					<template v-slot:after-options>
						<q-separator class="ob-text-accent-0" />
						<div v-if="isDateRangeVisible" class="q-px-sm">
							<q-item-section class="ob-text-accent-2 q-my-sm">Custom Range</q-item-section>
							<q-form ref="formDateRange" greedy>
								<div>
									<label>From</label>
									<q-input
										class="
											ob-border-accent-0 ob-border-radius-5 ob-input-field
											filter-date-field
											q-px-sm
											label-accent-2
										"
											...
											>
												...
												>
													<q-date
														...
														minimal
														flat
														class="ob-bg-accent-0 text-white"
														color="white"
														text-color="black"
													/>
												</q-popup-proxy>
								...
								<div class="q-mt-xs">
									...
									<q-input
										class="
											filter-date-field
											ob-border-radius-5 ob-input-field ob-border-accent-0
											label-accent-2
											q-px-sm
										"
										v-model="toDate"
							...
							<div class="q-mb-md q-mt-sm">
								<q-btn
									...
									class="apply-btn ob-border-radius-15 ob-bg-accent-0 border-white full-width"
									label="Apply"
						...
						<div v-else class="q-pa-sm">
							...
							>
								<template v-slot:append>
									<q-icon
										name="calendar_today"
										class="cursor-pointer margin-top-n13 ob-text-accent-2 text-14"
									>
										<q-popup-proxy
											ref="selectDate"
											cover
		...
		const filter = (action) => {
			...
			GetSearchResult({
				...
				$q.notify({
					...
					classes: `${status ? 'ob-success-notif' : 'ob-error-notif'} q-px-lg q-pt-none q-pb-none`,
					html: true,
					message: status
